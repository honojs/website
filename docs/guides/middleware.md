...
Note that if the handler or any middleware throws, hono will catch it and either pass it to [your app.onError() callback](/docs/api/exception#handling-httpexception) or automatically convert it to a 500 response before returning it up the chain of middlewares. This means that next() will never throw, so there is no need to wrap it in a try/catch/finally.
